...
def test_root_endpoint(client):
    ...
    assert response.status_code == 200
    data = response.json()
    assert data["service"] == "Assera API"
    assert "version" in data


@pytest.mark.integration
def test_assist_query_no_auth(client, sample_query_request):
...
@pytest.mark.integration
def test_assist_query_success(client, auth_headers, sample_query_request, mock_search_provider, mock_llm_client):
    """Test successful assist query"""
    with patch("app.main.search_provider", mock_search_provider), \
         patch("app.main.llm_client", mock_llm_client):
...
@pytest.mark.integration
def test_assist_query_with_session(client, auth_headers, mock_search_provider, mock_llm_client):
    ...
    with patch("app.main.search_provider", mock_search_provider), \
        ...
        session_id = response1.json()["session"]["session_id"]
...
@pytest.mark.integration
def test_assist_query_validation_error(client, auth_headers):
...
def test_models_select_global(client, auth_headers):
    ...
    response = client.post(
        ...
        json={"model": "test-model", "scope": "global"},
        headers=auth_headers,
    )
...
@pytest.mark.integration
def test_request_id_header(client, auth_headers, sample_query_request, mock_search_provider, mock_llm_client):
