...
"""
Configuration management for Assera API.
"""
...
class Settings(BaseSettings):
    ...
    api_title: str = "Assera Assisted API"
    api_description: str = "LLM-enhanced search API for Fess"
    debug: bool = Field(default=False, validation_alias="DEBUG")

    # Authentication
    assera_api_token: str = Field(..., min_length=32, validation_alias="ASSERA_API_TOKEN")

    # Search Provider
    assera_search_provider: Literal["fess"] = Field(
        default="fess", validation_alias="ASSERA_SEARCH_PROVIDER"
    )
    ...
    # LLM Provider
    assera_llm_provider: Literal["ollama"] = Field(
        default="ollama", validation_alias="ASSERA_LLM_PROVIDER"
    )
    assera_default_model: str = Field(default="gpt-oss", validation_alias="ASSERA_DEFAULT_MODEL")
    ollama_base_url: str = Field(default="http://ollama:11434", validation_alias="OLLAMA_BASE_URL")
    assera_llm_timeout_ms: int = Field(default=3000, validation_alias="ASSERA_LLM_TIMEOUT_MS")
    assera_llm_max_tokens: int = Field(default=512, validation_alias="ASSERA_LLM_MAX_TOKENS")
    assera_llm_temperature: float = Field(default=0.2, validation_alias="ASSERA_LLM_TEMPERATURE")
