  ...
  persist(
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        // Migration: Check for legacy 'intaste.token' key and migrate to new storage
        if (typeof window !== 'undefined' && state) {
          const legacyToken = localStorage.getItem('intaste.token');
          if (legacyToken && !state.apiToken) {
            // Migrate legacy token to new storage structure
            state.apiToken = legacyToken;
            // Remove legacy key
            localStorage.removeItem('intaste.token');
          }
        }
      },
    }
